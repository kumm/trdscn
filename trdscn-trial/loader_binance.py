...
def load():
    ...
    for smbl in all_symbol:
        startTime = smbl['last_append'] if smbl.get('last_append') is not None else 0
        print(f"Loading {smbl['sort']} "
              f"from {parse_timestamp(startTime).strftime('%Y-%m-%d %H:%M:%S')}...")
        candles = um_futures_client.klines(symbol=smbl['symbol'], interval="1d", limit=1500, startTime=startTime)
